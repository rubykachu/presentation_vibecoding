...
const TiltCard = ({ children, className }: { children: React.ReactNode; className?: string }) => {
  ...
  const rotateX = useTransform(y, [0, 1], [5, -5]); // Reduced tilt for readability
    ...
    >
      ...
      <div className="max-w-[1100px] mx-auto w-full z-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 w-full relative z-10">
          ...
          <div ref={codeRef} className="lg:col-span-8 h-full min-h-[500px] perspective-[1000px]">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 h-full text-left">

              {/* LEFT: THE CHAOS (Pain Point) */}
              <TiltCard className="bg-[#1a1010] border border-red-500/20 rounded-xl p-6 flex flex-col gap-6 relative overflow-hidden group hover:shadow-[0_0_30px_rgba(239,68,68,0.2)]">
                {/* Background Noise */}
                <div className="absolute inset-0 bg-[url('https://grainy-gradients.vercel.app/noise.svg')] opacity-10 pointer-events-none"></div>

                {/* 3D Floating Icon */}
                <div
                  className="absolute top-0 right-0 p-4 opacity-20 group-hover:opacity-100 transition-opacity"
                ...
                <div className="z-10" style={{ transform: "translateZ(20px)" }}>
                  <div className="flex items-center gap-2 mb-4">
                    <span className="px-2 py-1 bg-red-500/10 border border-red-500/30 rounded text-red-500 text-xs font-bold font-mono">VIBE_MODE: RANDOM</span>
                  </div>

                  <div className="space-y-4">
                    {/* The Vague Prompt */}
                    <div className="bg-white/5 p-4 rounded-lg border border-white/5">
                      <p className="text-xs text-slate-500 mb-1 font-mono">USER PROMPT:</p>
                      <p className="text-slate-300 italic">"Làm cái app quản lý giống Jira nha, nhưng đơn giản hơn, giao diện đẹp đẹp tí..."</p>
                    </div>
                    ...
                    <div className="flex flex-col gap-2">
                      <div className="flex items-center justify-between text-xs text-slate-400 font-mono">
                        ...
                        <span className="text-red-400">HALLUCINATING</span>
                      </div>
                    ...
                    <div className="space-y-2 mt-2">
                      <div className="flex items-center gap-3 text-sm text-red-300/80">
                        ...
                        <span>Logic auth lỏng lẻo (Hardcoded)</span>
                      </div>
                      <div className="flex items-center gap-3 text-sm text-red-300/80">
                        <XCircle className="w-4 h-4 shrink-0" />
                        <span>UI vỡ trên Mobile</span>
                      </div>
                      <div className="flex items-center gap-3 text-sm text-red-300/80">
                        <XCircle className="w-4 h-4 shrink-0" />
                        <span>Database không scale được</span>
                      </div>
                      <div className="flex items-center gap-3 text-sm text-red-300/80">
                        <XCircle className="w-4 h-4 shrink-0" />
                        <span>Không có Design System</span>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <TiltCard className="bg-[#0a1518] border border-[#2bcdee]/20 rounded-xl p-6 flex flex-col gap-6 relative overflow-hidden group hover:shadow-[0_0_30px_rgba(43,205,238,0.2)]">
                ...
                <div
                  ...
                  style={{ transform: "translateZ(30px)" }}
                >
                  <CheckCircle2 className="w-12 h-12 text-[#2bcdee]" />
                </div>

                <div className="z-10" style={{ transform: "translateZ(20px)" }}>
                  <div className="flex items-center gap-2 mb-4">
                    <span className="px-2 py-1 bg-[#2bcdee]/10 border border-[#2bcdee]/30 rounded text-[#2bcdee] text-xs font-bold font-mono">OPENSPEC_MODE: STRICT</span>
                  </div>

                  <div className="space-y-4">
                    {/* The Structured Input */}
                    <div className="bg-[#2bcdee]/5 p-4 rounded-lg border border-[#2bcdee]/20 font-mono text-xs">
                      <div className="flex justify-between mb-2 opacity-50">
                        <span>openspec.json</span>
                        <span>JSON Schema</span>
                      </div>
                      <div className="text-[#2bcdee] overflow-hidden whitespace-pre font-bold">
                    ...
                    <div className="flex flex-col gap-2">
                      <div className="flex items-center justify-between text-xs text-slate-400 font-mono">
                        <span>AI PROCESSING...</span>
                        <span className="text-[#2bcdee]">VALIDATED</span>
                      </div>
                      <div className="h-1 w-full bg-slate-800 rounded-full overflow-hidden">
                        <div className="h-full bg-gradient-to-r from-[#9d2bee] to-[#2bcdee] w-full"></div>
                      </div>
                    </div>

                    {/* The Result */}
                    <div className="space-y-2 mt-2">
                      <div className="flex items-center gap-3 text-sm text-[#2bcdee]">
                        <CheckCircle2 className="w-4 h-4 shrink-0" />
                        <span>Secure Auth Flow (Middleware)</span>
                      </div>
                      <div className="flex items-center gap-3 text-sm text-[#2bcdee]">
                        <CheckCircle2 className="w-4 h-4 shrink-0" />
                      ...
                      <div className="flex items-center gap-3 text-sm text-[#2bcdee]">
                        <CheckCircle2 className="w-4 h-4 shrink-0" />
                        <span>Scalable Schema (Drizzle)</span>
                      </div>
                      <div className="flex items-center gap-3 text-sm text-[#2bcdee]">
                        <CheckCircle2 className="w-4 h-4 shrink-0" />
